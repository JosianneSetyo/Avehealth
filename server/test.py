...
def error_weighted_average(weights, times, error=0.02):
    ...
    delta_t = np.diff(times_sec).astype(float)
    # Calculate the weights based on the time differences
    w_time = np.divide(1, delta_t, out=np.zeros_like(delta_t), where=delta_t!=0)
    print(w_time)
